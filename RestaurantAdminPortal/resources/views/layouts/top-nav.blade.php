<div class="flex justify-between mb-5">
      <div class="flex space-x-10 items-center ml-2">
            <span class="h3">{{auth()->user()->restaurant->name}}</span>
      </div>
            ...
            <div class="flex items-center">
                  <div id="top-nav-dropdown" class="dropdown show">
                        ...
                        <div class="dropdown-menu ml-3" aria-labelledby="dropdownMenu">
                              ...
                              <a class="dropdown-item" href="{{route('accountinfo.home')}}"><i class="fas fa-cog"></i>&nbsp;&nbsp; My Account</a>
                        </div>
                  </div>
...
<div class="{{(!session()->has('self_image_upload') || (!session()->has('success_menu_message')) && !session()->has('error_message')) ? 'hidden' : ''}} bg-{{session()->get('success_menu_message') ? 'green' : 'red'}}-500 text-{{session()->get('success_menu_message') ? 'green' : 'red'}}-200 py-2 text-center rounded-lg my-3">
      {{session()->has('success_menu_message') ? session()->get('success_menu_message') : session()->get('error_message')}}
</div>
